"""
Tier C4: REGIME_SWITCH_MIDWINDOW

Descriptive stress run with abrupt regime switch mid-window.
"""
...
from .runner import REPORTING_VERSION, aggregate_metrics, build_trader_params, write_scenario_outputs
...
EXPERIMENT_ID = "C4.REGIME_SWITCH_MIDWINDOW"
TIER = "C"
DEFAULT_BASE_SEED = 305000
DEFAULT_N_RUNS = 30
...
def simulate_market_switch(
    rng: np.random.Generator, cfg: MarketConfig, traders: List, p_low: float, p_high: float
) -> (List[Trade], List[tuple]):
    """Simulate with abrupt mid-window switch."""
    ...
    history: List[str] = []
    switch_step = cfg.n_steps // 2

    for t in range(cfg.n_steps):
        p_t = p_low if t < switch_step else p_high
...
def run_c4_regime_switch_midwindow(
    base_seed: int = DEFAULT_BASE_SEED,
    n_runs: int = DEFAULT_N_RUNS,
    p_low: float = 0.2,
    p_high: float = 0.8,
) -> List[Dict]:
    scenario_name = f"C4_regime_switch_midwindow__seed{base_seed}"
    ...
    cfg["tier"] = TIER
    cfg["p_true_low"] = float(p_low)
    cfg["p_true_high"] = float(p_high)
    cfg["sweep_params"] = {"p_true_low": cfg["p_true_low"], "p_true_high": cfg["p_true_high"]}
    ...
    for run_idx in range(n_runs):
        ...
        try:
            trade_tape, pool_path = simulate_market_switch(rng, market_cfg, traders, cfg["p_true_low"], cfg["p_true_high"])
            p_true_final = cfg["p_true_high"]
    ...
    results = {
        ...
        "tests": {},
        "warnings": warnings,
        "note": "Descriptive regime switch stress run.",
        "metadata": meta,
